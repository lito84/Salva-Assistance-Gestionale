...
$(document).ready(function(){
	...
	$(".salva-servizio").on("click", function(){
		$.post("actions/servizi.php",{id_area:$("#area").val(), servizio:$(".servizio_esistente").val(), nuovo_servizio:$("#nuovo_servizio").val(), sconto:$("#sconto").val(), prezzo_listino:$("#prezzo_listino").val(), prezzo_scontato:$("#prezzo_scontato").val(), id_circuito:"<?php echo $_GET["id_circuito"];?>", action:"aggiungi_servizio"}, function(data){
				$(".contenitore_servizi").empty().load("form/circuiti/servizi.php?id_circuito=<?php echo $_GET["id_circuito"];?>");
		})
	});
});	
</script>
